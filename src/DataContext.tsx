...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Ref para controlar o AbortController do fetch atual
  const abortControllerRef = useRef<AbortController | null>(null);

  // ── Carrega todos os dados do Supabase ──────────────────────────────────────
  const fetchFreshData = useCallback(async (showLoading: boolean, signal?: AbortSignal) => {
    if (showLoading) setLoading(true);

    try {
      // ── Fase 1: Dados essenciais (units, users, services, extras) ────────────
      const [
        { data: unitsData },
        { data: usersData },
        { data: servicesData },
        ...
        supabase.from('units')
          .select('id,name,address,phone,is_open,operating_hours')
          .is('deleted_at', null)
          .order('name')
          .abortSignal(signal),
        supabase.from('users')
          .select('id,auth_id,name,email,phone,role,unit_id,payment_type,daily_wage,base_commission_percent,lunch_value,transport_value,fixed_service_commissions,lavador_tipo')
          .is('deleted_at', null)
          .order('name')
          .abortSignal(signal),
        supabase.from('services')
          .select('id,name,unit_id,price_hatch,price_sedan,price_suv,price_pickup,duration_minutes,active,category')
          .is('deleted_at', null)
          .eq('active', true)
          .order('name')
          .abortSignal(signal),
        supabase.from('extras')
          .select('id,name,price,commission_value')
          .eq('active', true)
          .order('name')
          .abortSignal(signal),
      ]);

      // Se o fetch foi abortado (componente desmontou), sai silenciosamente
      ...
      // Libera a interface imediatamente após dados essenciais
      ...
      const [
        ...
        supabase.from('clients')
          .select('id,name,phone,unit_id,points,total_spent')
          .order('name')
          .limit(500)
          .abortSignal(signal),
        supabase.from('vehicles')
          .select('id,client_id,model,plate')
          .order('plate')
          .limit(500)
          .abortSignal(signal),
        supabase.from('appointments')
          .select('id,client_id,service_id,unit_id,washer_id,vehicle_type,plate,vehicle_model,start_time,end_time,status,total_price,selected_extras,client_name,photo_url,ai_data')
          .gte('start_time', ninetyDaysAgo)
          .order('start_time', { ascending: false })
          .limit(1000)
          .abortSignal(signal),
        supabase.from('transactions')
          .select('id,unit_id,type,amount,category,description,date,payment_method')
          .gte('date', ninetyDaysAgo)
          .order('date', { ascending: false })
          .limit(1000)
          .abortSignal(signal),
        supabase.from('production_records')
          .select('id,washer_id,unit_id,appointment_id,service_name,service_value,extras_data,commission_service,commission_extras,client_name,vehicle_plate,date,hour,status,extras_service_names,users(name)')
          .gte('date', ninetyDaysAgo)
          .order('date', { ascending: false })
          .limit(1000)
          .abortSignal(signal),
      ...
      const mappedProduction   = productionData?.map((row: any) =>
        mapProduction({ ...row, washer_name: row.users?.name })
      ) || [];
      ...
      // ── Salvar cache leve (sem dados pesados como photo_url e ai_data) ───────
      try {
        const cacheData = {
          timestamp: Date.now(),
          units: mappedUnits,
          ...
          // appointments, transactions e production ficam fora do cache
          // para não estourar o limite de ~5MB do sessionStorage
        };
        sessionStorage.setItem('app_cache_v2', JSON.stringify(cacheData));
      } catch (e) {
        // Se estourar, limpa apenas a chave de cache, não tudo
        sessionStorage.removeItem('app_cache_v2');
        sessionStorage.removeItem('app_cache');
      }

    } catch (err: any) {
      // Ignora erros de abort (são esperados ao desmontar o componente)
  ...
  const fetchAll = useCallback(async () => {
    ...
    const cachedRaw = sessionStorage.getItem('app_cache_v2') || sessionStorage.getItem('app_cache');
    if (cachedRaw) {
      try {
        ...
        if (cacheAge < 5 * 60 * 1000) {
          ...
          // appointments, transactions e production sempre frescos do servidor
          setLoading(false);

          // Atualiza tudo em background sem bloquear a UI
          fetchFreshData(false, controller.signal);
          return;
        }
      } catch (e) {
    ...
    await fetchFreshData(true, controller.signal);
  ...
  useEffect(() => {
    fetchAll();
    return () => {
      // Cancela qualquer fetch em andamento ao desmontar
      if (abortControllerRef.current) {
        abortControllerRef.current.abort();
      }
    };
  ...
  useEffect(() => {
    const channels = [
      supabase.channel('units-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'units' }, (p) =>
          setUnits(prev => prev.some(u => u.id === p.new.id) ? prev : [...prev, mapUnit(p.new)])
        )
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'units' }, (p) =>
          setUnits(prev => prev.map(u => u.id === p.new.id ? mapUnit(p.new) : u))
        )
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'units' }, (p) =>
          setUnits(prev => prev.filter(u => u.id !== p.old.id))
        )
        .subscribe(),

      supabase.channel('services-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'services' }, (p) =>
          setServices(prev => prev.some(s => s.id === p.new.id) ? prev : [...prev, mapService(p.new)])
        )
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'services' }, (p) =>
          setServices(prev => prev.map(s => s.id === p.new.id ? mapService(p.new) : s))
        )
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'services' }, (p) =>
          setServices(prev => prev.filter(s => s.id !== p.old.id))
        )
        .subscribe(),

      supabase.channel('extras-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'extras' }, (p) =>
          setExtras(prev => prev.some(e => e.id === p.new.id) ? prev : [...prev, mapExtra(p.new)])
        )
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'extras' }, (p) =>
          setExtras(prev => prev.map(e => e.id === p.new.id ? mapExtra(p.new) : e))
        )
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'extras' }, (p) =>
          setExtras(prev => prev.filter(e => e.id !== p.old.id))
        )
        .subscribe(),

      supabase.channel('clients-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'clients' }, (p) =>
          setClients(prev => prev.some(c => c.id === p.new.id) ? prev : [...prev, mapClient(p.new)])
        )
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'clients' }, (p) =>
          setClients(prev => prev.map(c => c.id === p.new.id ? mapClient(p.new) : c))
        )
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'clients' }, (p) =>
          setClients(prev => prev.filter(c => c.id !== p.old.id))
        )
        .subscribe(),

      supabase.channel('vehicles-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'vehicles' }, (p) =>
          setVehicles(prev => prev.some(v => v.id === p.new.id) ? prev : [...prev, mapVehicle(p.new)])
        )
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'vehicles' }, (p) =>
          setVehicles(prev => prev.map(v => v.id === p.new.id ? mapVehicle(p.new) : v))
        )
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'vehicles' }, (p) =>
          setVehicles(prev => prev.filter(v => v.id !== p.old.id))
        )
        .subscribe(),

      supabase.channel('users-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'users' }, (p) =>
          setUsers(prev => prev.some(u => u.id === p.new.id) ? prev : [...prev, mapUser(p.new)])
        )
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'users' }, (p) =>
          setUsers(prev => prev.map(u => u.id === p.new.id ? mapUser(p.new) : u))
        )
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'users' }, (p) =>
          setUsers(prev => prev.filter(u => u.id !== p.old.id))
        )
        .subscribe(),

      supabase.channel('appointments-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'appointments' }, (p) =>
          setAppointments(prev => prev.some(a => a.id === p.new.id) ? prev : [mapAppointment(p.new), ...prev])
        )
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'appointments' }, (p) =>
          setAppointments(prev => prev.map(a => a.id === p.new.id ? mapAppointment(p.new) : a))
        )
      ...
      supabase.channel('transactions-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'transactions' }, (p) =>
          setTransactions(prev => prev.some(t => t.id === p.new.id) ? prev : [mapTransaction(p.new), ...prev])
        )
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'transactions' }, (p) =>
          setTransactions(prev => prev.map(t => t.id === p.new.id ? mapTransaction(p.new) : t))
        )
      ...
      // ── production_records: trata cada evento individualmente ────────────────
      // CORRIGIDO: antes fazia fetch completo sem filtro em qualquer evento,
      // agora busca apenas o registro afetado (INSERT busca com JOIN, UPDATE/DELETE local)
      supabase.channel('production-changes')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'production_records' }, async (p) => {
          // Busca só o registro novo para obter o JOIN com users(name)
          ...
          if (data) {
            setProduction(prev =>
              prev.some(r => r.id === data.id)
                ? prev
                : [mapProduction({ ...data, washer_name: (data as any).users?.name }), ...prev]
            );
          }
        })
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'production_records' }, (p) => {
          setProduction(prev =>
            prev.map(r =>
              r.id === p.new.id
                ? mapProduction({ ...p.new, washer_name: p.new.washer_name || r.funcionarioNome })
                : r
            )
          );
        })
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'production_records' }, (p) => {
          setProduction(prev => prev.filter(r => r.id !== p.old.id));
        })
        .subscribe(),
    ];
  ...
  const addExtra = async (extra: Omit<Extra, 'id'>) => {
    ...
    setExtras(prev => prev.some(e => e.id === newExtra.id) ? prev : [...prev, newExtra]);
  };
  ...
  const addUser = async (user: Omit<User, 'id'>) => {
    ...
    setUsers(prev => prev.some(u => u.id === newUser.id) ? prev : [...prev, mapUser(newUser)]);
  };
  ...
  const updateUser = async (id: string, fields: Partial<User>) => {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (err.debug) console.error('Debug info:', err.debug);
      } catch {
  ...
  const addAppointment = async (appointment: Omit<Appointment, 'id'>) => {
    ...
    setAppointments(prev => prev.some(a => a.id === newAppt.id) ? prev : [newAppt, ...prev]);
  };
  ...
  const addTransaction = async (transaction: Omit<Transaction, 'id'>) => {
    ...
    setTransactions(prev => prev.some(t => t.id === newTransaction.id) ? prev : [newTransaction, ...prev]);
  };
  ...
  const addUnit = async (unit: Omit<Unit, 'id' | 'isOpen' | 'operatingHours'>) => {
    ...
    setUnits(prev => prev.some(u => u.id === newUnit.id) ? prev : [...prev, newUnit]);
  };
